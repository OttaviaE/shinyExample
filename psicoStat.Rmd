...
subtitle: "An introduction to shiny package"
...
shinyApp(
 ...
 ui = fluidPage(includeCSS("style.css"),
  ...
  sidebarLayout(
    sidebarPanel(style = "background-color: 		#e1e9f9;",
                    a(id = "imp_det", h3("Choose CSV file", style = "font-style: normal; font-size: 14pt;"), href = "#"),
          shinyjs::hidden(div(
            id = "details_import",
            helpText(
              h5("Choose a dataset. You can also upload your data!")
            )
          )),
      selectInput(inputId = "dataset",
                  label = "",
...
server = function(input, output){
 ...
 shinyjs::onclick("imp_det",
  ...
  output$graph <- renderPlot({
    ...
    ggplot(df, aes(x = df[,input$var1], 
                   y= df[,input$var2])) + geom_point()
    # plot(df[, c(1:2)]) # use it normally
